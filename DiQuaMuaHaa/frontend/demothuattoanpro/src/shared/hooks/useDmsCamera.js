...
import { getWebcamSupportErrorMessage } from "../contexts/cameraContext";
...
export function useDmsCamera({ onError, onStatusChange } = {}) {
  ...
  // Cleanup on unmount only (no auto-start - let parent control)
  useEffect(() => {
    return () => stopCamera();
  }, [stopCamera]);

  return { videoRef, streamRef, startCamera, stopCamera, isReady };
}
