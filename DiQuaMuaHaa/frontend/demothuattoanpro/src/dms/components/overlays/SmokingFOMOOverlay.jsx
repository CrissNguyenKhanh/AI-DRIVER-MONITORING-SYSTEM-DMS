...
export default function SmokingFOMOOverlay({ smokingDetectionRef, landmarksRef, videoRef }) {
  ...
  useEffect(() => {
    ...
    function draw() {
      ...
      ctx.setLineDash([5, 4]);
      ctx.strokeStyle = "rgba(255,80,40,0.55)"; ctx.lineWidth = 1.2;
      ...
      for (let i = 0; i < 4; i++) {
        const t = (st + i*0.25) % 1;
        ctx.beginPath();
        ctx.arc(x+w*0.6+Math.sin(t*Math.PI*4)*6, y-t*30, 2+t*3, 0, Math.PI*2);
