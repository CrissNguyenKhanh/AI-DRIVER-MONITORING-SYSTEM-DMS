...
import {
  FACE_OVAL_IDX, LIPS_IDX,
  L_EYE, R_EYE, LEFT_EYE_IDX, RIGHT_EYE_IDX,
} from "../../utils/geometry";
import { EAR_BLINK_THRESH } from "../../constants/dmsConfig";
...
function drawEye(ctx, lm, irisIdx, eyeIdxList, ear, W, H) {
  ...
  ctx.beginPath(); ctx.arc(ix, iy, iR*0.38, 0, Math.PI*2);
  ...
  for (let i = 0; i < 8; i++) {
    ...
    ctx.strokeStyle = blink ? "rgba(255,200,60,0.25)" : "rgba(0,180,255,0.22)";
...
export default function FaceMeshOverlay({ landmarksRef, eyeDataRef, videoRef }) {
  ...
  useEffect(() => {
    ...
    function draw() {
      ...
      ctx.closePath();
      ctx.strokeStyle = "rgba(100,180,255,0.35)"; ctx.lineWidth = 0.8; ctx.stroke();
      ...
      if (nT && nB) {
        ctx.beginPath(); ctx.moveTo(mx(nT.x), my(nT.y)); ctx.lineTo(mx(nB.x), my(nB.y));
        ctx.strokeStyle = "rgba(0,200,255,0.3)"; ctx.lineWidth = 0.8; ctx.stroke();
      }

      // bounding box
      const xs = lm.map(p => mx(p.x)).filter(Boolean);
      const ys = lm.map(p => my(p.y)).filter(Boolean);
      if (xs.length && ys.length) {
        const bx1 = Math.min(...xs), bx2 = Math.max(...xs);
        const by1 = Math.min(...ys), by2 = Math.max(...ys);
        const pad = 10, bx = bx1-pad, by = by1-pad;
        const bw = bx2-bx1+pad*2, bh = by2-by1+pad*2, cs = 18;
